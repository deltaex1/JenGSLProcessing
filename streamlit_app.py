...
st.set_page_config(page_title="Excel Transformer", page_icon="📊", layout="centered")
...
def gslreports(filename):
    ...
    df = df[[cols[0],cols[-2],cols[-1]]]  # Keep first, second-to-last, and last columns
    ...
    for name in names:
        output.loc[len(output.index)] = df[df[cols[0]] == name].nlargest(1, cols[-2]).iloc[0].tolist()
